...
const CONFIG_FILE: &str = ".wikivote.yml";
...
const WIKIVOTE_CHECK_NAME: &str = "WikiVote";
...
pub(crate) trait GH {
    ...
    /// Verify if the WikiVote check is required via branch protection in the
    ...
    async fn is_check_required(
            ...
            .and_then(|protection| protection.required_status_checks)
        {
            required_checks
                .contexts
                .iter()
                .any(|context| context == WIKIVOTE_CHECK_NAME)
        } else {
            false
        };
